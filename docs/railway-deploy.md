...
> 首次部署会执行 `prisma migrate deploy` + `seed`（写入测试账号与 5 万条审计数据），可能需要 **3~10 分钟**。
...
- 前端：https://asset-flow-web-production.up.railway.app
- API Health：https://asset-flow-api-production.up.railway.app/api/health
